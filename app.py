...
UNIT_SELLING_PRICE = 110 # Harder (was 120)
INITIAL_MARKET_PRICE = 60
HOLDING_COST = 3 # Harder (was 2)
SHORTAGE_PENALTY = 8 # Harder (was 5)
...
if st.session_state.phase == 'decision':
    ...
    with st.expander("ℹ️ 遊戲規則 (Game Rules)"):
        st.markdown(f"""
        - **目標:** 20 週內最大化現金與 KPI。
        - **KPI:** 缺貨扣 5 分，滿足加 2 分。低於 70/50 分會有高額罰款！
        - **需求:** 請參考「電影檔期表」預測需求，實際需求會有波動。
        - **上游限制:** 單筆訂單 > {UPSTREAM_CAPACITY} 單位，下週海運延誤 1 週。
        - **爆倉:** 超過 {WAREHOUSE_CAPACITY} 單位，每單位罰款 ${OVERFLOW_PENALTY}。
        """)
